<template>
  ...
  <div class="mainMenu">
      <Refine
            v-if="items.mainCategory.isOpen"
          @isOpen3="toggleChange"
          :categoryId=0
          :items="items"
          :isOpen="items.mainCategory.isOpen"
      ></Refine>
  </div>
  ...
  <div class="menu">
          <Refine
              v-if="subCategoryIsOpen"
              :categoryId="categoryId"
              @change3="handleMenuChange"
              @isOpen3="toggleChangeSubCategory"
              :items="items"
              :isOpen="isOpen"
          ></Refine>
      </div>
...
methods: {
  ...
  toggleChangeSubCategory({minorId,isOpen}){
    const item = this.items.majorCategory.find((item) => item.id === minorId);
  }
},
};
</script>
